;; ============================================================================
;; PREDIX v3 - Prediction Market (Gateway-Only, Price Bounds, Timelocks)
;; ============================================================================
;; Architecture changes from predixv2:
;; - Jackpot treasury on-chain: 1% of volume stays in contract, ticket logic off-chain
;; - place-bet: no "early" param (early window determined off-chain by sponsor)
;; - No public claim functions. Settlement is sponsor-only via gateway.
;; - resolve-and-distribute: atomic resolve + payout distribution (gateway-only)
;; - Counterparty check: no counterparty = refund without fee
;; - Price bounds on-chain: resolve rejects prices diverging >1% from last-known
;; - set-initial-price: deployer-only, one-shot bootstrap for price bounds
;; - Timelocks: schedule-gateway/schedule-sponsor with 144-block delay (~24h)
;; - Emergency withdraw: requires paused 200+ blocks, max 50% per execution
;; - TRADING_WINDOW reduced to u50 (10s pre-settlement window)
;; - Fee: 3% total split on-chain: 2% to fee-recipient, 1% stays as jackpot treasury
;; - Removed: user-pending-rounds, claim-round-side, claim-on-behalf, resolve-round
;; ============================================================================
...
(define-constant SELF 'ST1QPMHMXY9GW7YF5MA9PDD84G3BGV0SSJ74XS9EK.predixv7)
...
(define-constant PRICE_BOUND_BPS u100)       ;; 1% = 100 BPS
...
;; Timelocks - sponsor (stored in gateway, but predixv7 tracks for auditability)
...
;; Price bounds: rejects prices diverging >1% from last-known-price
...
(define-public (resolve-and-distribute (round-id uint) (price-start uint) (price-end uint))
  (let (
    ...
    ;; Price bounds: both prices must be within 1% of last-known
