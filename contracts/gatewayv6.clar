;; ============================================================================
;; GATEWAY v2 - Thin proxy for predixv7 (sponsor-only entry point)
;; ============================================================================
;; Changes from predixv2-gateway:
;; - Points to predixv7 (not predixv2)
;; - place-bet: no "early" param (early determined off-chain)
;; - resolve-and-distribute: new proxy function, sponsor-only
;; - SPONSOR data-var: configurable sponsor address for settlement auth
;; - All settlement restricted to sponsor wallet
;; ============================================================================
...
(define-public (place-bet (round-id uint) (side (string-ascii 4)) (amount uint))
  (begin
    ...
    (contract-call? .predixv7 place-bet round-id side amount)
  )
)
...
(define-public (resolve-and-distribute (round-id uint) (price-start uint) (price-end uint))
  (begin
    ...
    (contract-call? .predixv7 resolve-and-distribute round-id price-start price-end)
  )
)
...
(define-public (pay-jackpot-winner (winner principal) (amount uint))
  (begin
    ...
    (contract-call? .predixv7 pay-jackpot-winner winner amount)
  )
)
